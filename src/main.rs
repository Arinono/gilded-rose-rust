...
fn update_item_updates_all_items() {
    ...
    assert_eq!(updated_items[1].sell_in, 9);
    assert_eq!(updated_items[1].quality, 18);
}

#[test]
fn normal_items_decrease_in_quality_and_get_closer_to_sell_in_0() {
    let updated_item = update_item(&&Item { name: "Normal Item", sell_in: 10, quality: 20 });
    assert_eq!(updated_item.sell_in, 9);
    assert_eq!(updated_item.quality, 19);
}

#[test]
fn normal_items_decrease_quality_twice_as_fast_after_sell_in_date_passed() {
    let updated_item = update_item(&Item { name: "Normal Item", sell_in: 0, quality: 20 });
    assert_eq!(updated_item.quality, 18);
}

#[test]
fn normal_items_quality_can_never_be_negative() {
    let updated_item = update_item(&Item { name: "Normal Item", sell_in: 0, quality: 0 });
    assert_eq!(updated_item.quality, 0);
}

#[test]
fn aged_brie_gets_closer_to_sell_in_0() {
    let updated_item = update_item(&Item { name: "Aged Brie", sell_in: 10, quality: 20 });
    assert_eq!(updated_item.sell_in, 9);
}
...
fn aged_brie_increases_in_quality_as_it_gets_older() {
    let updated_item = update_item(&Item { name: "Aged Brie", sell_in: 10, quality: 20 });
    assert_eq!(updated_item.quality, 21);
}

#[test]
fn aged_brie_cannot_increase_past_quality_of_50() {
    let updated_item = update_item(&Item { name: "Aged Brie", sell_in: 10, quality: 50 });
    assert_eq!(updated_item.quality, 50);
}

#[test]
fn sulfuras_does_not_decrease_in_quality_or_sell_in() {
    let updated_item = update_item(&Item { name: "Sulfuras, Hand of Ragnaros", sell_in: 10, quality: 50 });
    assert_eq!(updated_item.sell_in, 10);
    assert_eq!(updated_item.quality, 50);
}

#[test]
fn backstage_passes_gets_closer_to_sell_in_0() {
    let updated_item = update_item(&Item { name: "Backstage passes to a TAFKAL80ETC concert", sell_in: 10, quality: 20 });
    assert_eq!(updated_item.sell_in, 9);
}

#[test]
fn backstage_passes_increase_in_quality_by_1_above_10_days_sell_in() {
    let updated_item = update_item(&Item { name: "Backstage passes to a TAFKAL80ETC concert", sell_in: 11, quality: 20 });
    assert_eq!(updated_item.quality, 21);
}
...
fn backstage_passes_increase_in_quality_by_2_below_10_days_sell_in() {
    let updated_item = update_item(&Item { name: "Backstage passes to a TAFKAL80ETC concert", sell_in: 10, quality: 20 });
    assert_eq!(updated_item.quality, 22);
}

#[test]
fn backstage_passes_increase_in_quality_by_3_below_5_days_sell_in() {
    let updated_item = update_item(&Item { name: "Backstage passes to a TAFKAL80ETC concert", sell_in: 5, quality: 20 });
    assert_eq!(updated_item.quality, 23);
}

#[test]
fn backstage_passes_become_0_quality_when_sell_in_passes() {
    let updated_item = update_item(&Item { name: "Backstage passes to a TAFKAL80ETC concert", sell_in: 0, quality: 20 });
    assert_eq!(updated_item.quality, 0);
}

#[test]
fn backstage_passes_cannot_increase_past_quality_of_50() {
    let updated_item = update_item(&Item { name: "Backstage passes to a TAFKAL80ETC concert", sell_in: 10, quality: 50 });
    assert_eq!(updated_item.quality, 50);
}

#[test]
fn conjured_mana_cake_gets_closer_to_sell_by_date() {
    let updated_item = update_item(&Item { name: "Conjured Mana Cake", sell_in: 10, quality: 20 });
    assert_eq!(updated_item.sell_in, 9);
}
...
fn conjured_mana_cakes_decrease_quality_twice_as_fast_after_sell_in_date_passed() {
    let updated_item = update_item(&Item { name: "Conjured Mana Cake", sell_in: 0, quality: 20 });
    assert_eq!(updated_item.quality, 16);
}

#[test]
fn conjured_mana_cake_quality_can_never_be_negative() {
    let updated_item = update_item(&Item { name: "Conjured Mana Cake", sell_in: 0, quality: 0 });
    assert_eq!(updated_item.quality, 0);
}